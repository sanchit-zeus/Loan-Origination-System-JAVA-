...
public class LoanDetails extends Database{
	...
	void creditscore() {
		...
		if(type.equals("education")) {
		    ...
		    System.out.println("EMI: (Rs)"+(loanamount*0.15/duration)/12+" per month");
		}
		else if(type.equals("personal")) {
			...
			System.out.println("EMI: (Rs)"+(loanamount*0.09/duration)/12+" per month");	
		}
		else if(type.equals("car")) {
			System.out.println("The loan granted is: (Rs)"+loanamount*.55*+creditscore*.1+" at 8% intrest");
			System.out.println("EMI: (Rs)"+(loanamount*0.08/duration)/12+" per month");
		}
	    }
		}
	}
